...
public:
    ...
    void setSampleRate(float sampleRate) {
        ...
        // NOTE: setCutoffFreq() early-returns when the cutoff is unchanged, so
        // filters whose cutoff is not re-set after setSampleRate keep their
        // previous coefficients. This is intentional historical behaviour and
        // part of the Golden reference — see RATE_CONTEXT.md. Do not "fix" it.
...
public:
    ...
    void setSampleRate(float sampleRate) {
        ...
        _1_sampleRate_pi = (1.0 / sampleRate) * -_2M_PI;
        _maxCutoffFreq = sampleRate / 2.0 - 1.0;
        // See the LP filter note: the early-return is historical behaviour.
        setCutoffFreq(_cutoffFreq);
        clear();
    }
