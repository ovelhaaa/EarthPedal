...
// Explicit sample-rate roles. See EarthRateContext.h / RATE_CONTEXT.md.
//
// - timingReferenceRate drives delay/tap lengths and the LFO excursion.
// - filterSampleRate drives the tank one-pole coefficients (the filters are
//   still *processed* at processSampleRate; that mismatch is the historical
//   identity, not a bug — do not "correct" it).
// - modulationSampleRate drives the LFO phase advance.
// - the freeze crossfade stays coupled to the tank timing rate, which keeps it
//   a constant real-time duration across host rates (legacy behaviour).
void Dattorro1997Tank::setRateContext(const earth::EarthRateContext& rc) {
    rateContext = rc;

    sampleRate = static_cast<float>(rc.timingReferenceRate);
    if (sampleRate < 1.f) sampleRate = 1.f;
    sampleRateScale = sampleRate / dattorroSampleRate;

    fadeStep = 1.f / sampleRate;

    const float filtRate = static_cast<float>(rc.filterSampleRate);
    leftOutDCBlock.setSampleRate(filtRate);
    rightOutDCBlock.setSampleRate(filtRate);
    leftHighCutFilter.setSampleRate(filtRate);
    rightHighCutFilter.setSampleRate(filtRate);
    leftLowCutFilter.setSampleRate(filtRate);
    rightLowCutFilter.setSampleRate(filtRate);

    const float modRate = static_cast<float>(rc.modulationSampleRate);
    lfo1.setSamplerate(modRate);
    lfo2.setSamplerate(modRate);
    lfo3.setSamplerate(modRate);
    lfo4.setSamplerate(modRate);
...
// Explicit-role variant. The outer input filters, pre-delay and input
// diffusion all-passes are referenced to the process rate; the tank receives
// the full context.
void Dattorro::setRateContext(const earth::EarthRateContext& rc) {
    rateContext = rc;

    const float processRate = static_cast<float>(rc.processSampleRate);
    sampleRate = processRate;
    preDelay = InterpDelay(static_cast<unsigned int>(std::ceil(sampleRate * kMaximumPreDelaySeconds)) + 1, 0.);

    tank.setRateContext(rc);
    dattorroScaleFactor = sampleRate / dattorroSampleRate;
    setPreDelay(preDelayTime);
    inApf1.delay.setDelayTime(dattorroScale(kInApf1Time));
    inApf2.delay.setDelayTime(dattorroScale(kInApf2Time));
